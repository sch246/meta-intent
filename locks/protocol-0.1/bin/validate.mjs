...
const verifierProtocol = { id: "meta-intent", revision: "0.1" }
...
async function validatePackage(inputRoot) {
  ...
  const currentLockPaths = new Set([...(localProtocolLockPath ? [localProtocolLockPath] : []), ...activePaths, ...candidatePaths])
  for (const lockPath of lockPaths) {
    const lock = await validateLock({
      ...
      stateDocumentPath,
      logPaths,
      isCurrent: currentLockPaths.has(lockPath),
    })
...
async function validateTopology(root) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isFile() && presentationFiles.has(entry.name)) continue
...
function validateStateShape(state) {
  if (state?.$schema !== "meta-intent/0.1/state") throw new Error("unsupported state schema")
...
async function validateLock(context) {
  const { root, lockPath, state, stateEntryPath, stateDocumentPath, logPaths, isCurrent } = context
  ...
  if (isCurrent && (lock.protocol.id !== state.protocol.id || lock.protocol.revision !== state.protocol.revision)) {
  ...
  if (isCurrent && (lock.package.id !== state.id || lock.package.revision !== state.revision)) {
  ...
  const lockedLogPaths = await validateLogBindings(root, dirname(lockPath), lock.logs, logPaths, isCurrent)
  ...
  if (lock.kind === "protocol") {
    ...
    requireString(lock.target?.kind, "realization target kind")
    requireString(lock.target?.identity, "realization target identity")
    requireString(lock.target?.baseline, "realization target baseline")
  }
...
async function validateStateBinding(root, base, binding, stateEntryPath, stateDocumentPath, state, lockPackage, isCurrent) {
  requireString(binding?.revision, "lock state revision")
  if (isCurrent && binding.revision !== state.revision) throw new Error("current lock state revision does not match current state")
...
async function validateLogBindings(root, base, bindings, selectedLogPaths, isCurrent) {
  ...
  if (isCurrent && (locked.size !== selectedLogPaths.length || selectedLogPaths.some((path) => !locked.has(path)))) {
    throw new Error("lock logs do not exactly match state logs")
  }
  return [...locked]
}
