...
Status: draft reference state. This package intentionally contains no target implementation code. It is derived from `/root/dsh-right-sidebar/PROPOSAL.md`, which remains the authority for the existing plugin repository until the user approves a cutover.
...
- Existing design evidence: `/root/dsh-right-sidebar/PROPOSAL.md`.
